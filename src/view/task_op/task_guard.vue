    ...
    <Row :gutter="14" span="24" >
      <i-col span="24" >
        <Card :title="$route.name" style="height: 790px;">
          ...
          <i-col span="24">
            <div id="allmap_guard" style="height: 700px; width: 100%; position: relative;"></div>
          </i-col>
            ...
            <Row :gutter="14" span="24" >
              <i-col :md="24" :lg="24">
                <Card v-if="drawer_vis" title="各客户产品总计工作时长及排名（h）" style="height: 340px;">
                  <guard-time-rate style="height: 250px;"></guard-time-rate>
                </Card>
              </i-col>
              <i-col :md="24" :lg="24">
                <Card v-if="drawer_vis" title="各客户产品巡逻次数及排名（次）" style="height: 340px; margin-top: 14px;">
                  <guard-total-rate style="height: 250px;"></guard-total-rate>
                </Card>
              </i-col>
            </Row>
          </drag-drawer>
...
import guardTimeRate from '_c/charts/guardTimeRate'
import guardTotalRate from '_c/charts/guardTotalRate'
...
export default {
  ...
  methods: {
    ...
    addMarker (point) {
      ...
      let myIcon = new BMap.Icon(use_img[parseInt(Math.random() * 1 + 3)], new BMap.Size(55, 55), {
  ...
  mounted () {
    // 百度地图API初始化
    let map = new BMap.Map('allmap_guard')
